...
public class PartsLoader {
    ...
    public static void init() throws IOException {
        ...
        for (Type type : Type.VALUES) {
            fromType(type).parts.add(new PerfPart("Empty", "", type, null, null,0, 0,0,0,0));
    ...
    private static void readPPart(Database database, VltClass vltClass) {
        ...
        for (VltCollection collection : collections) {
            ...
            if (splitName[0].equals("t0") && type != null && color != null && star != 0 && brand != null) {
                ...
                PerfPart perfPart = new PerfPart(star + "* " + color.name() + " " + brand.name(), collection.Name, type, color, brand, star, topspeed.GetValue(), acceleration.GetValue(), handling.GetValue(), 0);
    ...
    public PerfPart findPart(Color color, Brand brand, int star) {
        for (PerfPart part : VALUES) {
            if (part.color() == color && part.brand() == brand && part.star() == star) {
                return part;
            }
        }
        return null;
