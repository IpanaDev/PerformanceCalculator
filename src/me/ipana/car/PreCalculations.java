...
import me.ipana.car.value.*;

public class PreCalculations {
    public static IValue preCalculationsTire(boolean IsFront, AxlePairValue[] array) {
        boolean dynamic = false;
        AxlePairValue axlePair1 = array[0];
        for (int i = 1; i < array.length; i++) {
            AxlePairValue axlePair2 = array[i];
            if (axlePair2 != null && (IsFront ? axlePair1.Front != axlePair2.Front : axlePair1.Rear != axlePair2.Rear)) {
                dynamic = true;
                break;
            }
        }

        float firstNode = IsFront ? axlePair1.Front : axlePair1.Rear;
        if (dynamic) {
            float[] NEW_ARRAY = new float[4];
            for (int i = 0; i < NEW_ARRAY.length - 1; i++) {
                float node = IsFront ? array[i + 1].Front : array[i + 1].Rear;
