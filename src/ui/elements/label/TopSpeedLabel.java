...
public class TopSpeedLabel {
    ...
    public void calcAndSet(Result result, Car car) {
        ...
        //int tGain = Math.abs(result.engine().tGain()) + Math.abs(result.turbo().tGain()) + Math.abs(result.trans().tGain()) + Math.abs(result.suspension().tGain()) + Math.abs(result.brakes().tGain()) + Math.abs(result.tires().tGain());
        //int aGain = Math.abs(result.engine().aGain()) + Math.abs(result.turbo().aGain()) + Math.abs(result.trans().aGain()) + Math.abs(result.suspension().aGain()) + Math.abs(result.brakes().aGain()) + Math.abs(result.tires().aGain());
        //int hGain = Math.abs(result.engine().hGain()) + Math.abs(result.turbo().hGain()) + Math.abs(result.trans().hGain()) + Math.abs(result.suspension().hGain()) + Math.abs(result.brakes().hGain()) + Math.abs(result.tires().hGain());
        ...
        for (int i = 2; i < car.gearRatio().length; i++) {
            ...
            double speedKMH = (((RPM / (car.gearRatio()[i]) / FINAL_DRIVE) * TyreCircumference) / 1000000) * 60;
